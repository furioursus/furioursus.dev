...
- **`astro-mtg-collection` is a `github:`-sourced git dependency with its own `prepare` build
  ...
  one run — npm runs a git dependency's `prepare` script automatically on install, no allowlist
  needed. One less moving part since the switch off pnpm; see [`docs/README.md`](./README.md) if
  this package ever needs re-pinning to a different commit.
