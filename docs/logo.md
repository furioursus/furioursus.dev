...
## Edge distortion (removed)

`#Head`, `#BackEar1` and `#BackEar2` used to carry `filter: url(#logo-melt)` — an
`feTurbulence`/`feDisplacementMap` pair defined inline on the `<svg>` in `Header.astro`, with a
SMIL `<animate>` pulsing the displacement `scale` between 1 and 6 on a 5s loop. It gave the
silhouette a photocopy-static wobble.

**Removed 2026-09-13.** The displacement was large enough to completely swamp `logo-breathe`, which
moves the head by 0.8% horizontally and 1.5% vertically — the bear had been breathing the whole
time and nobody could see it under the static. Taking the filter off is what made the breath
visible, and that was the entire point of the change; the breathing animation itself was not
touched.

Three things went with it, and none of them exist any more:

- the `<defs>` block on the `<svg>` in `Header.astro` holding `#logo-melt`
- the `is:inline` script that removed `#logo-melt-pulse` under
  `prefers-reduced-motion: reduce` — SMIL ignores that media query, so it had to be done by hand.
  That whole class of problem is gone with the SMIL.
- the `filter` rule in `logo.css`

Worth keeping from the old note, in case anything like it is ever added back: the filter targeted
`#BackEar1`/`#BackEar2` directly rather than their `#Ear1`/`#Ear2` group wrappers, because SVG
filters apply to everything painted inside the filtered element's subtree — filtering the group
would have caught the pale ear-interior cutouts too and turned the whole mark fuzzy instead of just
its outer edge. An SVG `filter` is also a separate raster pass over the already-transformed shape,
